...
async function makeBoard(frames) {
  ...
  let markup = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
    ...
    <text x="40" y="58" font-family="Arial,sans-serif" font-size="32" font-weight="700" fill="#183B33">Avatar 1 · rebuilt idle views</text>
    ...
    <text x="40" y="816" font-family="Arial,sans-serif" font-size="16" fill="#596A62">IDLE DRAFT FOR APPROVAL — exact exported game cells. Animation awaits your separate approval.</text>`;
  const placements = [];
...
async function main() {
  ...
  check(decision.status === 'internal-refinement',
    'This generator only updates an internal-refinement draft. Preserve a rejected or approved review and explicitly start a new revision before changing its pixels.');
  ...
  const report = {
    ...
    mechanicalStatus: 'pass', userIdleApproval: 'not-granted', animationAcceptance: 'not-tested',
