"""Build editable v4 native Grease Pencil fills. Execute only via Blender MCP.

Supply AVATAR_SOURCE_DIR. The input is deliberately authored semantic vector
geometry; packed bitmap references are kept outside the export camera.
"""
import bpy
...
spec = importlib.util.spec_from_file_location('avatar_v4_contract', Path(AVATAR_SOURCE_DIR) / 'source_contract.py')
...
for ob in list(bpy.data.objects):
    if ob.name.startswith(prefix):
        contract.require(not ob.users_scene, 'A v4 object is shared with another scene; preserve it and resolve the ownership first')
...
def pack_reference(path, label, digest, position, approval):
    ...
    ref.empty_display_type = 'IMAGE'; ref.data = image; ref.empty_display_size = 96
...
pack_reference(data['conceptPath'], 'Approved concept reference', data['approval']['sha256'],
               (100, 1, 0), 'Approved concept identity; not sprite or animation approval')
for index, reference in enumerate(data['references']):
    pack_reference(reference['path'], 'ImageGen construction study NOT approved concept: ' + reference['path'].stem,
                   reference['sha256'], (100 + 110 * (index + 1), 1, 0), reference['approval'])
materials, indexes = [], {}
