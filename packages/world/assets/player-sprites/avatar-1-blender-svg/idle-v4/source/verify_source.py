#!/usr/bin/env python3
"""Check technical v4 geometry -> live GP readback -> native SVG parity.
...
def relative(path):
    # All report paths are relative to idle-v4, including ../concept evidence.
    import os
...
def inspect_environment(document, data, frames, readback):
    ...
    check(document['scene'] == 'AV1V4 | Idle authoring' and document['frame'] == 1 and
          document['render'] == [64, 64] and document['camera'] ==
          {'type': 'ORTHO', 'orthoScale': 64, 'location': [0, -128, 0]}, 'Environment framing changed')
...
def main():
    ...
    files.extend(data['gridProvenance']['files'])
    frames = {}
    for direction in data['directions']:
        ...
        if (ROOT / png).is_file():
            ...
            check(len(raw) >= 33 and raw[:8] == b'\x89PNG\r\n\x1a\n' and raw[12:16] == b'IHDR' and
                  struct.unpack('>II', raw[16:24]) == (64, 64), png + ': invalid 64x64 PNG header')
            frames[direction]['png'] = {'file': png, 'headerDimensions': [64, 64], 'sourcePixelParity': 'not-asserted'}
            files.append(png)
        else:
    ...
    report = {'schemaVersion': 1, 'revision': 'idle-v4', 'mechanicalStatus': 'pass',
              ...
              'limits': ['The coordinator performs reopening; this script does not independently open or parse .blend.',
                         'Upstream grid, study and applied-patch hashes are checked; the sampling, edits and tracing scripts are inventoried but not replayed here.',
              ...
              'upstreamProvenance': data['gridProvenance'],
