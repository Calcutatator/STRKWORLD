"""Export existing v4 GP artwork using Blender's native SVG operator via MCP.
...
cam.data.type = 'ORTHO'; cam.data.ortho_scale = 64; cam.data.shift_x = 0; cam.data.shift_y = 0
...
for direction in data['directions']:
    ...
    contract.require(float(svg.attrib['width'].removesuffix('px')) == 64 and
                     float(svg.attrib['height'].removesuffix('px')) == 64, 'Native SVG viewport changed')
...
report = {'schemaVersion': 1, 'blender': bpy.app.version_string, 'nativeMcpTools': True,
          'scene': scene.name, 'camera': {'type': cam.data.type, 'orthoScale': 64, 'location': list(cam.location)},
          'render': [64, 64], 'frame': 1, 'directions': data['directions'], 'exports': exports,
