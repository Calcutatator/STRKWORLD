...
def part_for(d,x,y,k,palette):
    ...
    teal=name.startswith('teal')
    if y<18:
        if skin or (name=='ink' and y>=13): return 'head.face'
        return 'head.hair'
    ...
    if y>=44: return 'boot.screen-left' if x<32 else 'boot.screen-right'
    if y>=36: return 'leg.screen-left' if x<32 else 'leg.screen-right'
...
def run():
    ...
    palette=grid['palette']; assert len(palette)<=24
    views={}
    for d,v in grid['views'].items():
        ...
        views[d]=shapes
    geometry={'cell':[64,64],'feetPivot':[32,56],'conceptSha256':approval['sha256'],
    ...
    (ROOT/'source/palette.json').write_text(json.dumps({'name':'Teal runner - bright shared production palette','maximumColorsPerFrame':24,'colors':palette},indent=2)+'\n')
