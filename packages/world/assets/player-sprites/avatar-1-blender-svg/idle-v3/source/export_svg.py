...
assert hashlib.sha256((root.parent/'concept'/approval['artifact']).read_bytes()).hexdigest()==approval['sha256']
...
report={'blender':bpy.app.version_string,'nativeMcpTools':True,'scene':scene.name,'camera':{'type':cam.data.type,'orthoScale':64,'location':list(cam.location)},'render':[64,64], 'frame':1,'exports':exports,'geometrySource':'source/authoring.py','regionRecipe':'source/build_geometry.py','reference':'../concept/avatar-1-concept-v2.png','conceptSha256':approval['sha256'],'construction':'source/idle-geometry.json','visibleGroupsOnly':True,'palette':'source/palette.json','master':'source/avatar-1.blend','approval':'pending','animation':'not-started'}
