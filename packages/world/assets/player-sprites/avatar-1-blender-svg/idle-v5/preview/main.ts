...
const textureKey = (facing: Facing) => `avatar-v5-${facing}`;
...
class DraftScene extends Phaser.Scene {
  ...
  constructor() { super({ key: 'avatar-v5-draft' }); }
  ...
  create() {
    try {
      ...
      for (const facing of FACINGS) {
        ...
        // Only this avatar changes sampling. The shared town builder explicitly
        // preserves NEAREST on the audited pixel-art tiles and door textures.
...
function inspectionEvidence() {
  ...
  body.updateFromGameObject();
  return {
    ready: true, stage: 'isolated-idle-draft', inspectedAt: new Date().toISOString(),
    ...
    avatar: { facing: scene.facing, lineup: scene.lineup, surface: scene.surface, sourceDensity: DENSITY, sourceCell: [SOURCE_CELL, SOURCE_CELL], logicalCell: [player.displayWidth, player.displayHeight], cssCell: [player.displayWidth * camera.zoom * css.width / canvas.width, player.displayHeight * camera.zoom * css.height / canvas.height], scale: [player.scaleX, player.scaleY], origin: [player.originX, player.originY], sourceFeet: [128, 224], logicalFeet: [32, 56], position: [player.x, player.y], physicsSourceSize: [body.sourceWidth, body.sourceHeight], physicsSourceOffset: [body.offset.x, body.offset.y], physicsWorldSize: [body.width, body.height], physicsCenter: [body.center.x, body.center.y], filters: Object.fromEntries(FACINGS.map((facing) => [facing, scene.textures.get(textureKey(facing)).source[0]!.scaleMode])) },
    ...
    sourceFrames: metadata.frames, previewSources: metadata.previewSources, errors: [...engineErrors],
  };
}
