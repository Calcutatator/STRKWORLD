"""Export existing v6 GP artwork using Blender's native SVG operator via MCP.
...
contract.require(scene['geometry_sha256'] == data['geometrySha256'] and
                 scene['palette_sha256'] == data['paletteSha256'] and
                 scene['concept_sha256'] == data['approval']['sha256'] and
                 scene['style_sha256'] == data['styleSha256'], 'Source inputs changed; explicitly rebuild before export')
...
report = {'schemaVersion': 1, 'blender': bpy.app.version_string, 'nativeMcpTools': True,
          ...
          'styleReference': '../reference/approved-concept-turnaround.png', 'styleSha256': data['styleSha256'],
          'pixelContract': {'sourceCell': [64, 64], 'pngCell': [64, 64], 'pngDensity': 1,
                            'feetPivot': [32, 56], 'maximumOpaqueColorsGlobal': 32, 'alpha': 'binary'},
