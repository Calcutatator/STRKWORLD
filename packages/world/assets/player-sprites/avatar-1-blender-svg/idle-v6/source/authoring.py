"""Build editable v6 native Grease Pencil fills. Execute only via Blender MCP.

Supply AVATAR_SOURCE_DIR. Pixel-cluster vector regions retain their declared
...
for ob in list(bpy.data.objects):
    if ob.name.startswith(prefix):
        contract.require(not ob.users_scene, 'A v6 object is shared with another scene; preserve it and resolve the ownership first')
...
scene['style_sha256'] = data['styleSha256']
scene['concept_approval'] = 'James: Yes good concept move forward'
scene['style_approval'] = 'Original approved medium JRPG sprite style; restored reference direction accepted'
...
scene['png_density'] = 1
scene['rendering_style'] = 'Medium JRPG pixel sprite; integer cluster edges, binary alpha, global 32-color maximum'
...
pack_reference(data['stylePath'], 'Approved medium JRPG sprite style reference', data['styleSha256'],
               (1680, 1, 0), 'Approved original sprite style and medium proportions; not approval of v6 pixels')
for index, reference in enumerate(data['references']):
    pack_reference(reference['path'], 'Internal construction study: ' + reference['path'].stem,
                   reference['sha256'], (2560 + 880 * index, 1, 0), reference['approval'])
materials, indexes = [], {}
...
for direction in data['directions']:
    ...
    ob['style_sha256'] = data['styleSha256']
...
print(json.dumps({'scene': scene.name, 'directions': data['directions'],
                  ...
                  'styleSha256': data['styleSha256'], 'approvedReferencesPacked': 2,
