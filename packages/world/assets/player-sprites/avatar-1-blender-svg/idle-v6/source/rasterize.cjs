 ...
 * never fits individual silhouettes or changes their height. Each integer-aligned region
 * rasterizes directly at native64; binary alpha is asserted. No resampling,
 * palette remapping or alpha thresholding is applied.
...
function measure(raw, label) {
  ...
  check(partialAlphaPixels === 0, `${label}: SVG has non-binary coverage; fix source alignment`);
...
async function makeBoard(frames) {
  ...
  let markup = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
    ...
    <text x="32" y="40" font-family="Arial,sans-serif" font-size="23" font-weight="700" fill="#203B35">Avatar 1 · JRPG sprite idle inspection</text>
    <text x="32" y="70" font-family="Arial,sans-serif" font-size="14" fill="#54685D">64² native pixel vectors → 64² texture · 64 × 64 logical cell</text>
  ...
  frames.forEach((frame, index) => {
    ...
    markup += `<text x="${left + columnWidth/2}" y="105" text-anchor="middle" font-family="Arial,sans-serif" font-size="16" font-weight="700" fill="#203B35">${frame.direction.toUpperCase()}</text>
      ...
      <text x="${left + 150}" y="445" text-anchor="middle" font-family="Arial,sans-serif" font-size="13" fill="#54685D">4× nearest enlargement · inspect pixel clusters</text>
  ...
  for (const panel of panels) {
    ...
    overlays.push({left:panel.left,top:panel.top,input:await sharp(frame.png)
      .resize(panel.size,panel.size,{kernel:'nearest'}).png().toBuffer()});
  }
...
async function main() {
  ...
  for (const direction of contract.directions) {
    ...
    const output = await sharp(inspected.canonical,{density:72,limitInputPixels:CELL*CELL})
      .toColourspace('srgb').ensureAlpha().resize(PNG_SIZE,PNG_SIZE,{kernel:'nearest'})
  ...
  const report = {schemaVersion:1,revision:'idle-v6',directions:contract.directions,
    ...
    registration:{sourceCell:[64,64],sourceFeet:[32,56],textureCell:[64,64],textureFeet:[32,56],
      logicalCell:[64,64],logicalFeet:[32,56],density:1},
    treatment:{rasterizer:'Sharp/libvips SVG rasterization at native 64x64; no downsampling',
      alpha:'binary alpha verified directly; no threshold or post-export cleanup',
      color:'exact authored sRGB fills; global32-color palette verified against the editable grid',
  ...
  process.stdout.write(`Rasterized ${frames.length} pixel-vector idle(s) to native 64² textures.\n`+
