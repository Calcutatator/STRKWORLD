#!/usr/bin/env python3
"""Check technical v6 geometry -> live GP readback -> native SVG parity.
...
def relative(path):
    # All report paths are relative to idle-v6, including both reference roles.
    import os
...
def inspect_readback(document, data, palette):
    ...
    check(document['geometrySha256'] == data['geometrySha256'] and
          ...
          document['styleSha256'] == data['styleSha256'] and
          document['pixelContract'] == {'sourceCanvas': 64, 'logicalCell': 64, 'pngDensity': 1} and
    ...
    expected_refs = {data['approval']['sha256']: 'Approved concept identity',
                     data['styleSha256']: 'Approved original sprite style and medium proportions'}
    expected_refs.update({ref['sha256']: 'Internal construction study' for ref in data['references']})
    packed = document['packedReferences']
    check(len(packed) == 2 + len(data['references']) and
...
def inspect_environment(document, data, frames, readback):
    ...
    check(document['scene'] == 'AV1V6 | Idle authoring' and document['frame'] == 1 and
    ...
    check(document['masterSha256'] == contract.sha256(ROOT / 'source/avatar-1.blend') and
          ...
          document['conceptSha256'] == data['approval']['sha256'] and
          document['styleSha256'] == data['styleSha256'] and
          document['styleReference'] == '../reference/approved-concept-turnaround.png' and
          document['pixelContract'] == {'sourceCell': [64, 64], 'pngCell': [64, 64], 'pngDensity': 1,
                                       'feetPivot': [32, 56], 'maximumOpaqueColorsGlobal': 32, 'alpha': 'binary'} and
          document['geometryProvenance'] == data['geometry'].get('provenance', {}),
...
def inside_contours(x, y, contours):
    """Even-odd test at pixel centers; source edges are integer and orthogonal."""
    inside = False
    for loop in contours:
        for start, end in zip(loop, loop[1:] + loop[:1]):
            if (start[1] > y) != (end[1] > y):
                intersection = start[0] + (y - start[1]) * (end[0] - start[0]) / (end[1] - start[1])
                if x < intersection:
                    inside = not inside
    return inside


def inspect_png(path, shapes, palette):
    with Image.open(path) as image:
        check(image.format == 'PNG' and image.size == (64, 64), str(path) + ': expected 64x64 PNG')
        check(getattr(image, 'n_frames', 1) == 1, str(path) + ': idle must contain exactly one frame')
        pixels = list(image.convert('RGBA').getdata())
    colors = set()
    for pixel in pixels:
        check(pixel[3] in (0, 255), str(path) + ': binary alpha required; no antialias cleanup is permitted')
        if pixel[3]:
            colors.add('#{:02X}{:02X}{:02X}'.format(*pixel[:3]))
    check(colors and colors <= set(palette.values()) and len(colors) <= contract.MAX_COLORS,
          str(path) + ': opaque PNG colors must come from the one global authored palette')
    expected = [(0, 0, 0, 0)] * (64 * 64)
    for shape in shapes:
        hex_color = palette[shape['color']]
        rgba = tuple(int(hex_color[i:i+2], 16) for i in (1, 3, 5)) + (255,)
        min_x = min(p[0] for loop in shape['contours'] for p in loop)
        max_x = max(p[0] for loop in shape['contours'] for p in loop)
        min_y = min(p[1] for loop in shape['contours'] for p in loop)
        max_y = max(p[1] for loop in shape['contours'] for p in loop)
        for y in range(min_y, max_y):
            for x in range(min_x, max_x):
                if inside_contours(x + 0.5, y + 0.5, shape['contours']):
                    expected[y * 64 + x] = rgba
    for index, (actual, wanted) in enumerate(zip(pixels, expected)):
        # RGB bytes hidden under fully transparent alpha are not visible art.
        check(actual[3] == wanted[3] and (not wanted[3] or actual[:3] == wanted[:3]),
              '{}: PNG/source pixel mismatch at {},{}'.format(path, index % 64, index // 64))
    return {'file': relative(path), 'dimensions': [64, 64], 'pngDensity': 1, 'alpha': 'binary',
            'opaqueColors': sorted(colors), 'opaqueColorCount': len(colors),
            'sourcePixelParity': 'pass',
            'comparison': 'All 4096 pixel centers compared against ordered source polygons with even-odd fills'}
...
def main():
    ...
    files = ['source/idle-geometry.json', 'source/palette.json', 'source/avatar-1.blend',
             ...
             '../concept/' + data['approval']['artifact'], '../reference/approved-concept-turnaround.png',
             '../reference/reference-manifest.json', 'review/user-decisions.json']
    files.extend(ref['relative'] for ref in data['references'])
    files.extend(data['constructionProvenance']['files'])
    frames, rendered_colors = {}, set()
    for direction in data['directions']:
        ...
        if (ROOT / png).is_file():
            frames[direction]['png'] = inspect_png(ROOT / png, data['geometry']['views'][direction], palette)
            rendered_colors.update(frames[direction]['png']['opaqueColors'])
            files.append(png)
        else:
    ...
    check(len(rendered_colors) <= contract.MAX_COLORS, 'Rendered PNGs exceed 32 opaque colors globally')
    report = {'schemaVersion': 1, 'revision': 'idle-v6', 'mechanicalStatus': 'pass',
              ...
              'spriteStyleAuthority': {'sha256': data['styleSha256'],
                                       'artifact': '../reference/approved-concept-turnaround.png',
                                       'scope': 'Original medium JRPG sprite style and proportions; no v6 pixel approval'},
              'pixelContract': {'cell': [64, 64], 'feetPivot': [32, 56], 'pngDensity': 1,
                                'maximumOpaqueColorsGlobal': 32, 'renderedOpaqueColorsGlobal': len(rendered_colors),
                                'alpha': 'binary', 'authoredPaletteColors': len(palette)},
              'artisticQuality': 'not-assessed', 'userIdleApproval': 'not-established',
              'animationAcceptance': 'not-tested', 'gameAcceptance': 'not-tested',
              'scope': 'Both reference bindings, geometry/palette to live GP readback and native SVG parity, plus exact PNG pixels when present',
              ...
              'limits': ['The coordinator performs reopening; this script does not independently open or parse .blend.',
                         'Construction-study hashes and declared pixel-art input files are checked; construction scripts are inventoried but not replayed here.',
                         'PNGs are optional during native source construction; every present PNG is checked against all 4096 source pixel centers.',
              ...
              'frames': frames, 'files': {name: fingerprint(ROOT / name) for name in dict.fromkeys(files)}}
