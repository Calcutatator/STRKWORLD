...
`/api/../health`, encoded dot segments, encoded slash/backslash, duplicate
slashes and semicolon path parameters therefore crossed into the private
...
body. It accepts origin-form `/api`, `/api/` and canonical `/api/v1/...` paths,
preserving queries unchanged, while rejecting literal/decoded dot segments,
encoded slash or backslash, duplicate slash, semicolon parameters, fragments,
invalid percent escapes and non-origin-form targets. Nearby `/apis` and `/api2`
...
*Verified:* deterministic raw-edge regressions first forwarded ten ambiguous
...
The focused Fly edge suite passes 31 tests, all Fly tests pass 4 files / 137
tests and the full workspace passes 102 files / 1,581 tests. Fly and workspace
typechecks, production build, 13 invariants, tilemap gate and diff hygiene pass.
