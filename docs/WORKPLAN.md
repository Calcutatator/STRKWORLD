...
| **Art** | `packages/world/assets` | Existing runtime assets remain current. Idle-v5 remains rejected and v1–v4 unapproved. Original concept identity/costume and medium sprite-style/direction approvals remain valid. The producer has withdrawn v6 as `withdrawn-quality-failed` after root and a fresh independent reviewer compared matched visible character heights and found noisy/spindly form, a muddy face and jagged legs. The previous artistic pass was wrong; exact userDecision remains null, with no user rejection inferred. Historical source-pixel parity and reviewed hashes remain technical evidence. Before any new aesthetic pass, compare the original reference and exact native output side by side at equal visible character height, then inspect the actual game output. Rebuild intentional native-pixel silhouette, face and major material clusters; point-sampling oversized pseudo-pixel ImageGen studies and patching only the face failed. No new method contract, revision parameters or ready candidate is claimed. Animation and production integration remain unapproved. See [active workflow](../packages/world/assets/player-sprites/avatar-1-blender-svg/README.md). |
