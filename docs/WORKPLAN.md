...
| **Art** | `packages/world/assets` | Existing runtime assets remain current. James approved exact fresh concept-v2 under D-059. Idle-v3 is withdrawn and unapproved after renewed aesthetic QA failed; its source/export parity remains technical evidence only. Rebuilt idle-v4 passed producing-agent and independent inspection of the actual 1×/2× exports after head refinement; its exact Blender/SVG/PNG review is frozen for separate user idle approval. The producing agent must pass visual quality and concept comparison at actual 1×/2× before another handoff. Animation, runtime integration and game acceptance remain later gates. See [active workflow](../packages/world/assets/player-sprites/avatar-1-blender-svg/README.md). |
