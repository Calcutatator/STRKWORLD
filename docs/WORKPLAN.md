...
| **Art** | `packages/world/assets` | Existing runtime assets remain current. James approved exact fresh concept-v2 under D-059. Idle-v3’s four Blender/SVG idles passed internal likeness review and saved-master export checks; the exact cells await separate user idle approval before animation. Historical idle-v2 remains superseded and unapproved. Game acceptance is a later gate. See [active workflow](../packages/world/assets/player-sprites/avatar-1-blender-svg/README.md). |
