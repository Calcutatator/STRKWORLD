...
| **Art** | `packages/world/assets` | Existing runtime assets remain current. James approved exact fresh concept-v2 under D-059. Idle-v3 and idle-v4 are withdrawn and unapproved; renewed v4 inspection found face/proportion quality below the concept, invalidating its artistic pass and handoff. Its source/export parity remains historical technical evidence only; v4 was never integrated or inspected in the live game. Further refinement awaits clarification between the crisp illustrated concept look and deliberate low-resolution pixel art while runtime presentation constraints are investigated. No replacement method or candidate is selected. The producing agent must pass actual-export aesthetic QA before another handoff. Animation, runtime integration and game acceptance remain later gates. See [active workflow](../packages/world/assets/player-sprites/avatar-1-blender-svg/README.md). |
