...
| **Art** | `packages/world/assets` | Existing runtime assets remain current. Exact concept-v2 is approved under D-059; v1–v4 remain rejected, superseded or withdrawn and unapproved. James has accepted higher-detail illustrated vector artwork at the same on-screen size. Idle-v5 is a producer-inspected draft pending exact user idle approval: 512×512 native Blender/SVG authoring, 256×256 PNGs at density 4, 64×64 logical frame and 24×24 world body, smooth alpha and linear avatar filtering. An isolated actual-Phaser preview with real town assets at zoom 2 and necessary browser visual QA are authorized. Before every handoff, the producer must personally inspect the source and actual engine output at real game CSS scale across turns and light/dark surfaces; visible defects require more refinement. Current source and actual-engine visual inspection passed for this idle draft; exact user approval remains pending. Evidence and pre-handoff freshness verification are stored with idle-v5. Explicit idle approval precedes animation; animated-art and production integration acceptance remain later gates. See [active workflow](../packages/world/assets/player-sprites/avatar-1-blender-svg/README.md). |
