...
| **Art** | `packages/world/assets` | Existing runtime assets remain current. James approved exact fresh concept-v2 under D-059. New idle-v3 Blender/SVG construction and internal likeness QA are in progress for four idles only; separate user idle approval is required before animation. Historical idle-v2 remains superseded and unapproved. Game acceptance is a later gate. See [active workflow](../packages/world/assets/player-sprites/avatar-1-blender-svg/README.md). |
