...
| **Art** | `packages/world/assets` | Existing runtime assets remain current. Idle-v5 is rejected and its artistic pass/approval request withdrawn under the 2026-09-15 D-059 correction; v1–v4 remain unapproved. Approved concept-v2 governs character identity/costume. The originally approved medium Avatar 1 sprite reference governs warm lower-resolution 16-bit JRPG style, proportions and elevated game viewpoint; it is distinct from fresh concept art. An internal sprite design translation gate now precedes four-idle production, checking medium proportions, simple expressive face, deliberate clusters, clean contours and limited detail against both authorities. Native Blender/SVG and actual Phaser inspection remain required tools, not artistic passes. The user approved the corrected reference direction on 2026-09-15. Idle-v6 has passed producer source and actual Phaser visual review and awaits exact four-idle approval: native64 grid, 50px visible target, shared32-color ceiling (27 rendered colors), binary alpha and nearest filtering. Native Blender/SVG/PNG parity and road/pavement/grass game-size captures are recorded. No exact-idle approval is granted. The producer must keep refining mismatches before handoff; exact idle approval, animation and production integration remain later gates. See [active workflow](../packages/world/assets/player-sprites/avatar-1-blender-svg/README.md). |
