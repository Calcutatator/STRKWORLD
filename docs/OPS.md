...
lockfile. `.dockerignore` excludes `.env*`, `*.pem` and `*.key` from the
context — a secret that enters a build context survives in image layers and in
`docker history`.
...
- **Probing a real route would be actively harmful.** Every request takes a
  ...
  (`apps/backend/src/api.ts:96`, D-026), and a probe every 30 seconds is a
...
(`apps/backend/src/metrics.ts:17`) and is exempted from the rate limiter and
...
- `FEE_AUTHORIZATION_SECRET` must be unchanged across the rollback, or every
  ...
  (`apps/backend/src/authorization.ts:68`) and players see failures rather than
  a clean rollback.
...
|---|---|---|
| Global | `BackendConfig.globalEnabled = false` | every private route returns `503 SERVICE_DISABLED` (`apps/backend/src/api.ts:107-109`). The city stays up; financial doors lock. |
| Per route | `routes.<transfer\|unshield\|swap>.enabled = false` | that building alone locks. `RoutePolicy.enabled`, `apps/backend/src/types.ts:19`. |
...
- No request-logging middleware is enabled, because there is no middleware
  ...
  not identity.
- `AggregateMetrics` (`apps/backend/src/metrics.ts:1-27`) holds six integer
...
constructor throws — `apps/backend/src/authorization.ts:48`). It signs the
...
secret (`apps/backend/src/authorization.ts:48`).
...
(`packages/privacy/src/wallet-api/backend-client.ts:14`) takes a base URL and
...
5. **No metrics exposure.** `AggregateMetrics.snapshot()`
   (`apps/backend/src/metrics.ts:17`) exists and is on no route, so the
   D-026 alert on `budgetExhausted` has nothing to read.
6. **Multi-instance caveat (already noted in D-026).** `AggregateRateLimiter`
   and `AggregateBudget` (`apps/backend/src/metrics.ts:58`, `:36`) are
